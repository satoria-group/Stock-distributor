...
@if (session('status') || session('success'))
    ...
    @endphp
    <div x-data="{ show: true }" x-show="show" class="mb-5 rounded-2xl border px-4 py-3.5 flex items-center justify-between gap-3 text-xs font-semibold shadow-2xs transition {{ $isError ? 'bg-rose-50 text-rose-800 border-rose-200' : 'bg-emerald-50 text-emerald-800 border-emerald-200' }}">
        <div class="flex items-center gap-3">
            @if ($isError)
                <svg width="18" height="18" style="width: 18px; height: 18px; min-width: 18px; flex-shrink: 0;" class="text-rose-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
        ...
        <button type="button" @click="show = false" class="{{ $isError ? 'text-rose-400 hover:text-rose-700 hover:bg-rose-100/60' : 'text-emerald-400 hover:text-emerald-700 hover:bg-emerald-100/60' }} cursor-pointer p-1 rounded-lg transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
            </svg>
        </button>
